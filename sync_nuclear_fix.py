...
# Files to sync
files_to_sync = [
    ('DjangoWebProject1/DjangoWebProject1/settings.py', 'DjangoWebProject1/settings.py'),
    ...
    ('DjangoWebProject1/app/static/app/js/quick-view.js', 'app/static/app/js/quick-view.js'),
]
...
try:
    ...
    # 3. FORCE CLEAR STATIC FILES AND RECOLLECT
    print("\n>>> NUCLEAR STATIC REFRESH...")
    # Clear WhiteNoise cache and manifest
    ssh.exec_command(f'docker exec {container_name} rm -rf /app/staticfiles/*')
    ssh.exec_command(f'docker exec {container_name} python manage.py collectstatic --noinput --clear')
    
    # 4. Restart container
    print("Restarting web container to apply ALL changes...")
    ssh.exec_command(f'docker restart {container_name}')
    
    print("\nULTIMATE FIX APPLIED SUCCESSFULLY!")
    print("ACTION REQUIRED: Please clear your browser cache (Ctrl+F5) and check again.")
